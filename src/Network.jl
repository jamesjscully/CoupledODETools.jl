...
function generate(n, mod = Main)
    ...
    return (f = Core.eval(mod, f), fcode = f, u0 = u0, pType = pType, uType = uType)
end
function generate_ensemble(n, mod = Main)
    ...
    u0 = Float32[n.icarr...]
    return (f = Core.eval(mod, f), fcode = f, u0 = u0, space = space, idxs = idxs)
end
